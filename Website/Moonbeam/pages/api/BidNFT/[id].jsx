...
export default async function handler(req, res) {
  ...
  async function fetchContractData() {
    if (contract) {
      // try {
      const tokenURIJSON = await contract.tokenURI(id).call();
      ...
      const totalraised = await contract.getEventRaised(eventId).call();
      let Raised = 0;
      Raised = Number(totalraised) + Number(biddingPrice);
      const result2 = await contract.createBid(id, JSON.stringify(createdObject), JSON.stringify(parsed),senderAddress, eventId, Raised.toString()).send({
        from: senderAddress,
        gasPrice: 1000000000,
        gas: 5_000_000,
      });

...
export default function User() {
  ...
  function ProfileImage() {
    if (ProfileDetails?.profileImage !== undefined) {
      if (ProfileDetails.profileImage.length !== 0) {
        return (
          <><img src={`https://2eff.lukso.dev/${ProfileDetails?.profileImage[0].url.replace(':/', '',)}`} className="user UserImage" /></>
        )
      }
    }

    return (
      <><img src={`https://up-sample-web-app.staging.lukso.dev/img/lukso.507e62f5.png`} className="user UserImage" /></>
    )
  }
    ...
    <div className="user won-nft-Container">
      ...
      {(NFTs.length === 0) ? (<><h6>No items</h6></>) : (
        <div className="gap-3 won-nft-grid user">
          {NFTs.map((listItem) => (<NFTComponent key={listItem.TokenId} Id={listItem.Id} EventID={listItem.EventID} name={listItem.name} price={listItem.price} isGifted={listItem.isGift} image={listItem.image} signer={signerAddress} wallet={PROFILE_ADDRESS} showingFunc={activateGiftCardModal} unwrapingFunc={activateUnwrapGiftCardModal} />))}
        </div>)}
    </div>
